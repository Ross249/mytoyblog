...
public class CommentController extends BaseController {
    ...
    public String index(@RequestParam(value = "page",defaultValue = "1") int page,
        ...
        PageInfo<CommentVo> commentPaginator = commentService.getCommentsWithPage(commentVoExample, page, limit);
    ...
    public RestResponseBo delete(@RequestParam Integer coid){
        try {
            CommentVo comments = commentService.getCommentById(coid);
            if (null == comments){
    ...
    public RestResponseBo reply(@RequestParam Integer coid,@RequestParam String content, HttpServletRequest request){
        ...
        CommentVo c = commentService.getCommentById(coid);
        if (null == c){
            return RestResponseBo.fail("不存在该评论");
        }
        UserVo users = this.user(request);
