...
//        }
        ...
        if (StringUtils.isNotBlank(author) && author.length() > 50){
            return RestResponseBo.fail("名字过长");
        }
        ...
        if (null != count && count > 0){
            return RestResponseBo.fail("您发表评论太快了，请稍后再试");
        }
        ...
        try{
            ...
            cookie("table_remember_author", URLEncoder.encode(author,"UTF-8"),7 * 24 * 60 * 60,response);
            cookie("table_remember_mail", URLEncoder.encode(mail,"UTF-8"),7 * 24 * 60 * 60,response);
            if (StringUtils.isNotBlank(url)){
                cookie("table_remember_url", URLEncoder.encode(url,"UTF-8"),7 * 24 * 60 * 60,response);
            }
            // 设置对每个文章1分钟可以评论一次
